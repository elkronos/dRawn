# One test per finding in the pre-CRAN audit of the original sampleR scripts.
# Each reproduces the exact case that used to return a wrong answer. Every
# assertion here was confirmed to FAIL against the original implementation
# before the corresponding fix landed.

test_that("A1: a single numeric cluster id is not reinterpreted as a range", {
...
test_that("A2: capping a result no longer discards whole strata", {
...
test_that("A2: a temporal sample covers the whole window (invariant, not a guard)", {
...
test_that("A3: multistage allocation totals the request, up or down", {
  df <- make_df()
...
test_that("A4: the caller's weights come back untouched", {
...
test_that("A5: the lightest row stays selectable", {
...
test_that("A6: balanced clustering works with unused factor levels", {
  d <- data.frame(x = 1:30,
...
test_that("A7: block replicates are stitched from blocks across the frame", {
...
test_that("A8: bootstrap returns exactly n_replicates of equal size (invariant)", {
...
test_that("A9: POSIXct and Date columns are used as-is", {
  expect_silent(
    res <- draw(make_ts(), design_temporal("ts", "2020-01-01", "2020-01-02",
...
test_that("A10: a region list of three or more geometries is unioned", {
...
test_that("A11: count arguments reject negative, non-numeric and length-2 values", {
...
test_that("A12: single-column frames survive intact", {
...
test_that("A13: a short stream is trimmed, not padded with NULLs", {
  res <- draw(as.list(1:5), design_reservoir(n = 10), seed = 1)
  expect_equal(length(res), 5)
  expect_false(any(vapply(res, is.null, logical(1))))
})

test_that("A14: empty results keep the input's columns", {
  df <- make_df()
...
test_that("A15: NA cluster labels never fabricate rows", {
  d <- data.frame(x = 1:30, cl = c(rep("a", 10), rep("b", 10), rep(NA, 10)))

  # `==` used to return 20 rows for cluster "a", ten of them all-NA.
  ...
  # The guard used to read the NA-inflated count of 20 and let this through,
  # returning 15 rows from a 10-row cluster with replace = FALSE.
  expect_error(
...
test_that("A16: an antimeridian-spanning region warns instead of failing mutely", {
  skip_if_not_installed("sf")
  df <- make_df()
  expect_warning(
...
test_that("A16: the warning does not fire on legitimate multi-part regions", {
  skip_if_not_installed("sf")
  d <- data.frame(id = 1:3, lon = c(5, 25, 45), lat = c(5, 25, 45))
...
test_that("B2: allocation hits the total and does not floor rare strata", {
  uneven <- data.frame(
    x = 1:143,
...
test_that("B3: month buckets follow the calendar, not 30.44-day durations", {
  df <- data.frame(
    id = 1:365,
...
test_that("C1: count arguments are validated rather than passed to sample()", {
...
test_that("E5: drawing leaves the caller's RNG stream alone", {
  df <- make_df()
  set.seed(42)
...
test_that("E10: column order is preserved by every design", {
  df <- make_df()
  designs <- list(
    design_simple(n = 4),
...
test_that("removed arguments stay removed", {
  ...
  # them here: this is the real guard for the three findings whose defects are
