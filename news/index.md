...
Sampling is expressed as a design you build and then apply, rather than
a function call with the design buried in its arguments:
...
A design can be printed, stored, reused across data sets, and — the part
...
gives the first-order inclusion probability of every population row
under a design, without drawing anything — a quick way to check a plan
before committing to it. `draw(weights = TRUE)` attaches those
probabilities and the corresponding design weights to a sample as
`.prob` and `.weight`.
...
Not every design has a closed-form inclusion probability, and the ones
that don’t report that rather than returning an approximation:
...
[`design_weighted()`](https://elkronos.github.io/dRawn/reference/design_weighted.md)
offers three methods, and they are not interchangeable:
...
The default is what `base::sample(prob = )` does: a good way to bias
selection toward heavy units, but the weights govern each sequential
draw rather than the probability of ending up in the sample. Use
`"systematic"` or `"poisson"` when the sample will be estimated from.
Units heavy enough that `n * p_i > 1` are taken with certainty and the
remainder rescaled, iterating until every probability is valid.
...
- Under spherical geometry a longitude/latitude polygon whose edge spans
  ...
  [`design_spatial()`](https://elkronos.github.io/dRawn/reference/design_spatial.md)
  warns when a region has such an edge.
- Reservoir sampling takes a vectorised path for a data frame, whose
  length is already known, and Algorithm L for a genuine stream — a
  connection or a generator function.
- Calendar units in
  [`design_temporal()`](https://elkronos.github.io/dRawn/reference/design_temporal.md)
  step by calendar months and years, not by fixed 30.44-day durations.
